    ...
    for (lineno, raw) in text.lines().enumerate() {
        ...
        if let Some(flag) = fields.next() {
            if flag == "official" {
                ...
                return Err(format!(
                    "repos:{}: unknown flag '{flag}' (only 'official' is allowed)",
                    lineno + 1
                ));
            }
        }
        if let Some(extra) = fields.next() {
            return Err(format!("repos:{}: unexpected extra field '{extra}'", lineno + 1));
        }

        out.push(Repo { name: name.to_string(), url, priority, official });
    }
