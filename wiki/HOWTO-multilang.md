...
`--config-dir`). Su Slackware installi il **pacchetto binario**: mette tutto al
suo posto — l'eseguibile, la pagina man e i file di configurazione sotto
`/etc/slacker/`. **Non copi nulla.** Prima del primo uso bastano due modifiche:
scegli un mirror in `mirrors` e imposta le priorità dei repository in `repos`.
...
Dichiara i tuoi repository in `/etc/slacker/repos`. Il campo URL è un URL
letterale, la parola chiave `mirror` (il mirror attivo, per quello ufficiale) o
`mirror/<subpath>` (il mirror attivo con un sottopercorso aggiunto, es.
`mirror/extra` — segue un sottoalbero della distribuzione sul mirror scelto, senza
fissare l'host). I repository binari devono avere priorità **distinte**; vince la
priorità più alta:
...
`slacker history` stampa un registro, dal più recente, di ogni cambiamento di
pacchetto sul sistema — installato, aggiornato, reinstallato, rimosso — con la
data locale e il repository/tag di origine. È ricostruito dalle directory
amministrative di pkgtools sotto `ADM_DIR` (`packages/` + `removed_packages/`),
quindi riflette anche le modifiche fatte da altri strumenti (slackpkg, sbopkg,
installpkg/upgradepkg/removepkg), non solo da slacker.

```
slacker history                 # tutto, dal più recente
slacker history emacs           # un solo pacchetto (nome esatto)
slacker history --installed     # solo ciò che è installato ora, con la data
slacker history --removed       # solo ciò che ha lasciato il sistema
slacker history --upgraded      # solo aggiornamenti / reinstallazioni
slacker history --last 20       # i 20 eventi più recenti
slacker history --since 2026-06-01   # eventi dal giorno indicato in poi
```

Un aggiornamento si legge come `vecchio → nuovo`. I filtri si combinano. L'output
passa per un pager su terminale (`q` per uscire).
...
В Slackware вы устанавливаете **двоичный пакет**: он всё кладёт на место —
исполняемый файл, man-страницу и файлы конфигурации в `/etc/slacker/`. **Ничего
копировать не нужно.** Перед первым запуском достаточно отредактировать два файла:
выбрать зеркало в `mirrors` и задать приоритеты репозиториев в `repos`.
...
Объявите репозитории в `/etc/slacker/repos`. Поле URL — это либо обычный URL,
либо ключевое слово `mirror` (активное зеркало, для официального), либо
`mirror/<subpath>` (активное зеркало с добавленным подпутём, напр. `mirror/extra`
— отслеживает поддерево дистрибутива на выбранном зеркале, не зашивая хост).
Двоичные репозитории должны иметь **различные** приоритеты; побеждает наивысший:
...
`slacker history` печатает журнал (сначала новые) каждого изменения пакетов в
системе — установка, обновление, переустановка, удаление — с локальной датой и
репозиторием/тегом источника. Он восстанавливается из административных каталогов
pkgtools под `ADM_DIR` (`packages/` + `removed_packages/`), поэтому отражает и
изменения, сделанные другими инструментами (slackpkg, sbopkg,
installpkg/upgradepkg/removepkg), а не только slacker.

```
slacker history                 # всё, сначала новые
slacker history emacs           # только один пакет (точное имя)
slacker history --installed     # только установленное сейчас, с датой
slacker history --removed       # только покинувшее систему
slacker history --upgraded      # только обновления / переустановки
slacker history --last 20       # 20 последних событий
slacker history --since 2026-06-01   # события с указанной даты
```

Обновление читается как `старое → новое`. Фильтры комбинируются. Вывод проходит
через пейджер в терминале (`q` для выхода).
...
La configuración está en `/etc/slacker/` (se sobrescribe con `--config-dir`). En
Slackware instalas el **paquete binario**: deja todo en su sitio — el ejecutable,
la página de manual y los archivos de configuración bajo `/etc/slacker/`. **No
copias nada.** Antes del primer uso solo hay que editar dos archivos: elige un
mirror en `mirrors` y fija las prioridades de los repositorios en `repos`.
...
Declara tus repositorios en `/etc/slacker/repos`. El campo URL es una URL
literal, la palabra clave `mirror` (el mirror activo, para el oficial) o
`mirror/<subpath>` (el mirror activo con un subpath añadido, p. ej.
`mirror/extra` — sigue un subárbol de la distribución en el mirror elegido, sin
fijar el host). Los repositorios binarios deben tener prioridades **distintas**;
gana la más alta:
...
`slacker history` imprime un registro, del más reciente al más antiguo, de cada
cambio de paquete en el sistema — instalado, actualizado, reinstalado, eliminado —
con la fecha local y el repositorio/tag de origen. Se reconstruye a partir de los
directorios administrativos de pkgtools bajo `ADM_DIR` (`packages/` +
`removed_packages/`), así que también refleja cambios hechos por otras
herramientas (slackpkg, sbopkg, installpkg/upgradepkg/removepkg), no solo slacker.

```
slacker history                 # todo, lo más reciente primero
slacker history emacs           # un solo paquete (nombre exacto)
slacker history --installed     # solo lo instalado ahora, con su fecha
slacker history --removed       # solo lo que salió del sistema
slacker history --upgraded      # solo actualizaciones / reinstalaciones
slacker history --last 20       # los 20 eventos más recientes
slacker history --since 2026-06-01   # eventos desde una fecha en adelante
```

Una actualización se lee como `viejo → nuevo`. Los filtros se combinan. La salida
pasa por un paginador en terminal (`q` para salir).
...
配置位于 `/etc/slacker/`（可用 `--config-dir` 覆盖）。在 Slackware 上你安装**二进制
软件包**：它会把一切放到位——可执行文件、man 手册页，以及 `/etc/slacker/` 下的配置
文件。**无需复制任何文件。** 首次使用前只需编辑两个文件：在 `mirrors` 中选一个镜像，
并在 `repos` 中设置仓库优先级。
...
在 `/etc/slacker/repos` 中声明你的仓库。URL 字段可以是字面 URL、关键字 `mirror`
（当前镜像，用于官方仓库），或 `mirror/<subpath>`（当前镜像后附加一个子路径，例如
`mirror/extra`——在你所选的镜像上跟踪某个发行版子树，而无需写死主机名）。二进制仓库
的优先级必须**各不相同**；优先级最高者胜出：
...
`slacker history` 以最新在前的顺序打印系统上每一次软件包变更的日志——安装、升级、
重装、删除——并附上本地日期与来源仓库/tag。它从 `ADM_DIR` 下的 pkgtools 管理目录
（`packages/` + `removed_packages/`）重建而来，因此也会反映由其他工具（slackpkg、
sbopkg、installpkg/upgradepkg/removepkg）所做的变更，而不仅是 slacker。

```
slacker history                 # 全部，最新在前
slacker history emacs           # 仅某个软件包（精确名称）
slacker history --installed     # 仅当前已安装的，附安装日期
slacker history --removed       # 仅已离开系统的
slacker history --upgraded      # 仅升级 / 重装事件
slacker history --last 20       # 最近 20 个事件
slacker history --since 2026-06-01   # 自某日期起的事件
```

升级显示为 `旧 → 新`。各过滤器可组合。在终端中输出会经过分页器（按 `q` 退出）。
...
Sur Slackware tu installes le **paquet binaire** : il met tout en place — l'exécu-
table, la page de manuel et les fichiers de configuration sous `/etc/slacker/`.
**Tu ne copies rien.** Avant la première utilisation, il n'y a que deux fichiers à
éditer : choisis un miroir dans `mirrors`, et règle les priorités des dépôts dans
`repos`.
...
Déclare tes dépôts dans `/etc/slacker/repos`. Le champ URL est soit une URL
littérale, soit le mot-clé `mirror` (le miroir actif, pour l'officiel), soit
`mirror/<subpath>` (le miroir actif avec un sous-chemin ajouté, p. ex.
`mirror/extra` — il suit un sous-arbre de la distribution sur le miroir choisi,
sans coder le serveur en dur). Les dépôts binaires doivent avoir des priorités
...
`slacker history` imprime un journal, du plus récent au plus ancien, de chaque
changement de paquet sur le système — installé, mis à jour, réinstallé, supprimé —
avec la date locale et le dépôt/tag d'origine. Il est reconstruit à partir des
répertoires administratifs de pkgtools sous `ADM_DIR` (`packages/` +
`removed_packages/`), il reflète donc aussi les changements faits par d'autres
outils (slackpkg, sbopkg, installpkg/upgradepkg/removepkg), pas seulement slacker.

```
slacker history                 # tout, du plus récent
slacker history emacs           # un seul paquet (nom exact)
slacker history --installed     # seulement l'installé actuel, avec la date
slacker history --removed       # seulement ce qui a quitté le système
slacker history --upgraded      # seulement les mises à jour / réinstallations
slacker history --last 20       # les 20 événements les plus récents
slacker history --since 2026-06-01   # les événements à partir d'une date
```

Une mise à jour se lit `ancien → nouveau`. Les filtres se combinent. La sortie
passe par un pager sur un terminal (`q` pour quitter).
