...
3. Αναζήτηση, επιθεώρηση, λίστα αποθετηρίων και ιστορικό πακέτων
4. Εγκατάσταση
5. Αναβάθμιση
...
Στο Slackware εγκαθιστάς το **δυαδικό πακέτο** (χτισμένο από το SlackBuild). Αυτό
τα βάζει όλα στη θέση τους — το εκτελέσιμο `slacker`, τη σελίδα man, και τα αρχεία
ρυθμίσεων κάτω από το `/etc/slacker/` (η διαδρομή παρακάμπτεται με `--config-dir`).
**Δεν αντιγράφεις τίποτα.** Πριν την πρώτη χρήση χρειάζεται να επεξεργαστείς μόνο
δύο αρχεία: διάλεξε mirror στο `mirrors`, και όρισε τις προτεραιότητες των
αποθετηρίων στο `repos`.
...
Δήλωσε τα αποθετήριά σου στο `/etc/slacker/repos`. Το πεδίο URL είναι είτε ένα
κανονικό URL, είτε η λέξη-κλειδί `mirror` (ο ενεργός mirror, για το επίσημο), είτε
`mirror/<subpath>` (ο ενεργός mirror με ένα subpath στο τέλος, π.χ. `mirror/extra`
— ακολουθεί ένα υποδέντρο της διανομής στον mirror που διάλεξες, χωρίς να καρφώνεις
τον host). Τα δυαδικά αποθετήρια πρέπει να έχουν **διακριτές** προτεραιότητες·
νικά η υψηλότερη:
...
Το `slacker history` τυπώνει ένα χρονολόγιο, με τα νεότερα πρώτα, κάθε αλλαγής
πακέτου στο μηχάνημα — εγκατάσταση, αναβάθμιση, επανεγκατάσταση, αφαίρεση — με την
τοπική ημερομηνία της κάθε αλλαγής και το αποθετήριο/tag προέλευσης. Ανακατασκευά-
ζεται από τους admin καταλόγους του pkgtools κάτω από το `ADM_DIR` (`packages/` +
`removed_packages/`), οπότε αντικατοπτρίζει και αλλαγές από άλλα εργαλεία
(slackpkg, sbopkg, σκέτο installpkg/upgradepkg/removepkg), όχι μόνο του slacker.

```
slacker history                 # τα πάντα, νεότερα πρώτα
slacker history emacs           # μόνο ένα πακέτο (ακριβές όνομα)
slacker history --installed     # μόνο ό,τι είναι εγκατεστημένο τώρα, με ημερομηνία εγκατάστασης
slacker history --removed       # μόνο ό,τι έφυγε (αφαιρέθηκε ή αναβαθμίστηκε)
slacker history --upgraded      # μόνο αναβαθμίσεις / επανεγκαταστάσεις
slacker history --last 20       # τα 20 πιο πρόσφατα γεγονότα
slacker history --since 2026-06-01   # γεγονότα από μια ημερομηνία και μετά
```

Μια αναβάθμιση διαβάζεται ως `παλιό → νέο`. Τα φίλτρα συνδυάζονται (π.χ. `history
--installed --since 2026-06-01`). Η έξοδος περνά από pager σε terminal (πάτα `q`
για έξοδο). Σημείωση: επειδή τα σκέτα records του `removepkg` μοιράζονται ένα όνομα
αρχείου ανά package id στο `removed_packages`, μια μεταγενέστερη αφαίρεση
αντικαθιστά μια προηγούμενη· όταν χαθεί έτσι το record του στόχου μιας αναβάθμισης,
η νέα έκδοση συμπεραίνεται από την επόμενη γνωστή εγγραφή του πακέτου αντί να
εμφανιστεί `?`.
